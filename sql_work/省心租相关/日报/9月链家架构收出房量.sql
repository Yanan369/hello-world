--模板查询：【9月日报】链家事业部大区收出房量底表
--模板查询：【9月日报】链家事业部大区收出房量底表
--模板查询：【9月日报】链家事业部大区收出房量底表
-- 链家事业部大区收出房量底表
SELECT  case when emp.partname is null and emp.region_name is not null then emp.region_name
	   		 when emp.partname is null and emp.region_name is null and emp.dabu is not null then emp.dabu
			 when emp.partname is null and emp.region_name is null and emp.dabu is null then emp.corp_name else emp.partname end  as partname
...
(
	...
	(
		...
		-- AND job_name IN ( '租赁经纪人', '租赁店经理', '综合经纪人', '综合店经理')
	...
	(
		...
		(
			...
			AND contract_code not IN ('TG0000492342') 
		    -- AND trusteeship_housedel_code in ('107105749583','107105727433')
		) AS shou1
		LEFT JOIN
		(
			SELECT  *
...
ON emp.team_name = base.team_name
GROUP BY  emp.corp_name
		 ,emp.dabu
         ,emp.region_name
         ,emp.partname
grouping sets ((emp.corp_name,emp.dabu,emp.region_name,emp.partname),(emp.corp_name,emp.dabu,emp.region_name),(emp.corp_name,emp.dabu),(emp.corp_name))